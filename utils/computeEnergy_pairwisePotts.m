function [energy, dataTerm, smoothnessTerm] = computeEnergy_pairwisePotts(dataCost, neighbors, labels)
%computeEnergy_pairwisePotts computes the value of pairwise energy with Potts potentials
% 
% [energy, dataTerm, smoothnessTerm] = computeEnergy_pairwisePotts(dataCost, neighbors, labels)
%
% INPUT
...
if ~isnumeric(dataCost) || ~ismatrix(dataCost)
    error('computeEnergy_pairwisePotts:badDataCost', 'dataCost should be a matrix  numLabels x numNodes');
end
...
if ~isnumeric(neighbors) || ~ismatrix(neighbors) || ~issparse(neighbors) || size(neighbors, 1) ~= numNodes || size(neighbors, 2) ~= numNodes
    error('computeEnergy_pairwisePotts:badNeighbors', 'neighbors should be a sparse matrix numNodes x numNodes');
end

if ~isnumeric(labels) || ~iscolumn(labels) || length(labels) ~= numNodes || any(~ismember( unique(labels), 1 : numLabels))
    error('computeEnergy_pairwisePotts:badLabels', 'labels should be a column vector of length numNodes, 1-indexing for label IDs');
end
